...
async function executeBatchSubrequest(baseUrl, batchPart) {
    ...
    config.url = `${baseUrl}\\sap\\opu\\odata\\sap\\Z_S4Z0001_SRV\\${config.url}`;
...
function wrapSingleRequestResponse(response) {
    ...
    //     'Content-Type: multipart/mixed; boundary=9AF244D903F4401055098657640B86C30',
    //     `Content-Length: ${response.headers['content-length']}`,
    //     'dataserviceversion: 2.0',
    //     'cache-control: no-store, no-cache',
    //     '',
